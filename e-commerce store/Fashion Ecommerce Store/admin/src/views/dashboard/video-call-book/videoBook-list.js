...
const BookDataList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const response = await axiosInstance.get('/get-video-call-data');
        setInquiry(response.data.data);
        setIsLoading(false);
      } catch (error) {
        setIsLoading(false);
        console.error('Error fetching Video call book data:', error);
        toast.error('Error fetching Video call book data');
      }
    };
  ...
  const displayUsers = currentInquiry.map((users, index) => (
    <tr key={index}>
      <td>{index + 1}</td>
      <td>{users.name}</td>
      <td>{users.slot} {users.slot_time}</td>
      <td>{users.mobile}</td>
      <td>{users.email}</td>
      <td>
        <div className="flex align-items-center list-user-action">
          <Link
            ...
            to={`/video-book/view?id=${users._id}`}
          >
            ...
            <Card>
              <Card.Header>
                <div className="header-title d-none d-md-block">
                  <h4 className="card-title">Book Video Call List</h4>
                </div>
              </Card.Header>
                  ...
                  >
                    <thead>
                      <tr className="ligth">
                        <th><b>No.</b></th>
                        <th><b>Full Name</b></th>
                        <th><b>Video Call Date and Time</b></th>
                        <th><b>Mobile</b></th>
                        <th><b>Email</b></th>
                        <th><b>Action</b></th>
                      </tr>
