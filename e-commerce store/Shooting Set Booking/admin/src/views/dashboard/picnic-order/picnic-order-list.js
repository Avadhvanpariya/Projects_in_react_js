...
const OrderList = () => {
  ...
  const fetchOrderList = async () => {
    try {
      setIsLoading(true);
      const response = await axiosInstance.get('/get-picnic-order', {
        params: {
      ...
      if (response.data.status === 200) {
        ...
        toast.error('Error fetching order list: ' + response.data.message);
        setIsLoading(false);
      }
    } catch (error) {
      console.error('Error fetching order list:', error);
      toast.error('Error fetching order list: ' + error.message);
      setIsLoading(false);
    }
  };
  ...
  const filterOrderList = (orders) => {
    return orders.filter((order) => {
      const nameMatch = order.party_name.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleRemoveOrder = (orderId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await axiosInstance.post('/remove-picnic-order', { id: orderId });

          if (response.data.status === 200) {
            toast.success('Order Picnic removed successfully');
            setOrderList((prevOrderList) =>
              prevOrderList.filter((order) => order._id !== orderId)
            );
          } else {
            console.error('Error removing Picnic Order:', response.data.message);
            toast.error('Error removing Picnic Order: ' + response.data.message);
          }
        } catch (error) {
          console.error('Error removing Picnic order:', error);
          toast.error('Error removing Picnic order: ' + error.message);
        }
      }
    });
  };
            ...
            <Card>
              <Card.Header className="d-flex justify-content-between">
                <div className="header-title">
                  <h4 className="card-title">Picnic List</h4>
                </div>
                <div className="header-title">
                  <Link to="/dashboard/picnic-add" className="btn btn-btn btn-primary">
                    Create Picnic Order
                  </Link>
                </div>
              </Card.Header>
              <Card.Body className="px-0" style={{ position: 'relative' }}>
                ...
                <div className="table-responsive">
                  ...
                  >
                    ...
                    <tbody>
                      {filterOrderList(orderList).map((item, index) => (
                        <tr key={item._id}>
                          <td>{index + 1}</td>
                          <td>{item.party_name}</td>
                          <td>{item.mobile}</td>
                          <td>{item.picnic_date}</td>
                          ...
                          <td>
                            <div className="flex align-items-center list-user-action">
                              <Link
                                ...
                                to={`/dashboard/picnic-view?id=${item._id}`}
                              >
