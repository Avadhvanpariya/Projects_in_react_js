...
const BookinList = () => {
  ...
  const fetchUserList = async () => {
    try {
      setIsLoading(true);
      const response = await axiosInstance.get('/get-scan-data', {
        params: {
  ...
  const filterUserList = (users) => {
    ...
    return users.filter((user) => {
      ...
      // Check if the scan_time_date is equal to today
      const isToday = user.scan_time_date.split(' ')[0] === today;
            ...
            <Card>
              <Card.Header className="d-flex justify-content-between">
                <div className="header-title">
                  <h4 className="card-title">Scanning List</h4>
                </div>
              </Card.Header>
                    ...
                    <tbody>
                      {filterUserList(userList).map((item, index) => (
                        <tr key={item._id}>
                          ...
                          <td>{item.days}</td>
                          <td>{item.bookDate}</td>
                          <td>{item.scan_time_date}</td>
                          <td>
                            <div className="flex align-items-center list-user-action">
                              <Link
                                className="btn btn-sm btn-icon btn-success"
                                to={`/dashboard/scanning-view?id=${item._id}`}
                              >
